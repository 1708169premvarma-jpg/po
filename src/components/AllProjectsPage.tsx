...
// All projects data (copied from ProjectsSection)
...
export default function AllProjectsPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <header className="fixed top-0 left-0 w-full z-50 bg-black/95 backdrop-blur-lg border-b border-white/10 flex items-center px-4 py-4 md:px-12 md:py-6 shadow-lg">
        <button
          onClick={handleBack}
          className="mr-4 px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20 text-lg font-medium transition-all"
          aria-label="Back"
        >
          &#8592; Back
        </button>
        <h1 className="text-4xl md:text-5xl font-bold text-center w-full">ALL  PROJECTS </h1>
      </header>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto px-4" style={{ paddingTop: '7rem' }}>
        {allProjects.map((project, index) => (
          ...
          >
            <div
              className="relative aspect-[3/4] rounded-2xl overflow-hidden mb-4 max-w-xs mx-auto cursor-pointer"
        ...
        <div className="fixed inset-0 z-50 bg-black/95 backdrop-blur-sm overflow-y-auto" onClick={() => setSelectedProject(null)}>
          <div className="min-h-screen p-6 md:p-12 flex items-center justify-center">
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 50 }}
              className="max-w-5xl mx-auto w-full bg-black rounded-2xl border border-white/10 shadow-2xl p-6 md:p-12 relative"
              onClick={e => e.stopPropagation()}
            >
              {/* Close Button */}
              <button
                onClick={() => setSelectedProject(null)}
                className="ml-auto mb-6 p-3 bg-white/10 backdrop-blur-md rounded-full hover:bg-white/20 transition-colors flex items-center gap-2 absolute top-4 right-4"
              ...
              <div className="mb-8">
                ...
                <div className="flex items-center justify-between mb-4 gap-4">
                  <h2 className="text-5xl md:text-7xl">{selectedProject.title}</h2>
                  <button
                    onClick={() => setSelectedProject(null)}
                    className="p-2 bg-white/10 hover:bg-white/20 rounded-full transition-colors flex items-center gap-2 text-lg"
                    aria-label="Close"
                  >
                    <X size={24} />
